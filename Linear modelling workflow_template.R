...
choose.dir()
setwd("E:/Análisis_R/IMG/RN_IMG/gatosCT-IMG")
...
df <- read.table(file = "E:/Análisis_R/IMG/data/eventosgatos18-21.csv",      # Replace "myfile.csv" with your data file name.
...
#================================#
var_resp <- "eventos"                     # Replace with the name of the response variable.
...
var_fac <- c("etapa", "zona")   # Template: replace entries with the name(s) of your factor predictor(s).
...
# NUMERIC or INTEGER TIME variable that specifies temporal structure.
  ...
  var_time <- "etapa"                          # assign default NA if missing.
...
#------------------------------------------#
barplot_fac(data = df, variables = c(var_fac)) # Barplots for all factor predictors and random terms.
...
#------------------------------------------------#
corvif(data = df, variables = c(var_fac))  # VIF are calculated for all fixed predictors potentially included in the model.
...
mod <- glmmTMB(eventos ~ etapa + zona,         # Template: Replace with your complete model formula.
               data = df,                          # Name of dataframe.
               family = nbinom2(link = "log"), # Template: Replace with your distribution family and link.
               offset=log(esfuerzo))     
...
residual_plots_predictors(data = df,
                          modelTMB = mod,
                          predictors = c(var_fac)) # Name of all fixed predictors in the dataframe.
...
residual_plots_interactions(data = df, 
                            modelTMB = mod, 
                            predictors = c(var_fac))      # All fixed predictors in the dataframe.
...
plot_predictors <- c("etapa", "zona")      # Template: Replace with the names of MAX 2 predictor variable(s).
...
mod_predictions <- post_predict(data = df,
                                ...
                                offset = offset,                     # (optional) - Name of response offset variable, if present in the model. Default to NA.
